#!/usr/bin/env python3
"""
Generate the Figure 3.6b optimal-time heatmap from externally regenerated (g, Delta) sweep outputs.
"""
import numpy as np
...
def load_results(results_dir):
    ...
    g_values, delta_values, optimal_time_values, max_snr_values, is_monotonic = [], [], [], [], []
    for npz_file in npz_files:
        try:
            ...
            max_snr = snr[max_idx]; optimal_kappa_tau = kappa_tau[max_idx]
            mono = max_idx >= len(snr) - int(0.01 * len(snr))
            g_values.append(g); delta_values.append(delta)
            optimal_time_values.append(optimal_kappa_tau); max_snr_values.append(max_snr)
            is_monotonic.append(mono)
        except Exception as e:
            print(f"Error loading {npz_file.name}: {e}")
    return (np.array(g_values), np.array(delta_values),
            np.array(optimal_time_values), np.array(max_snr_values), np.array(is_monotonic))

def create_heatmap(g_vals, delta_vals, optimal_time_vals, max_snr_vals, is_monotonic, output_dir):
    unique_g = np.sort(np.unique(g_vals))
    unique_delta = np.sort(np.unique(delta_vals))
    time_grid = np.full((len(unique_g), len(unique_delta)), np.nan)
    ...
    for g, delta, opt_time, snr, mono in zip(g_vals, delta_vals, optimal_time_vals, max_snr_vals, is_monotonic):
        i, j = g_to_idx.get(g), delta_to_idx.get(delta)
        if i is not None and j is not None:
            if mono:
                time_grid[i, j] = np.nan; snr_grid[i, j] = np.nan
            else:
                time_grid[i, j] = opt_time; snr_grid[i, j] = snr
    delta_2pi = unique_delta / (2 * np.pi)
    g_2pi = unique_g / (2 * np.pi)

    fig, ax = plt.subplots(figsize=(8, 6))
    im = ax.imshow(time_grid, aspect='auto', origin='lower',
                   extent=[delta_2pi.min(), delta_2pi.max(), g_2pi.min(), g_2pi.max()],
                   cmap='viridis', interpolation='nearest')
    cbar = plt.colorbar(im, ax=ax)
    cbar.set_label(r'$\kappa\tau$', fontsize=22)
    ...
    plt.tight_layout()
    plt.savefig(Path(output_dir) / "optimal_time_heatmap.pdf", format='pdf', dpi=300, bbox_inches='tight')
    plt.close()
    print("Saved optimal_time_heatmap.pdf")
...
def main():
    ...
    g_vals, delta_vals, optimal_time_vals, max_snr_vals, is_monotonic = load_results(RESULTS_DIR)
    if len(g_vals) == 0:
        print("No results found!"); return
    create_heatmap(g_vals, delta_vals, optimal_time_vals, max_snr_vals, is_monotonic, OUTPUT_DIR)
