#!/usr/bin/env python3
"""
Gamma Sweep - Sweep over gamma from 0.001 to 0.1 in steps of 0.005

Parameters:
    kappa   = 2*pi*0.05
    epsilon = 2*pi*0.05
    gamma   = sweep parameter (0.001 to 0.1)
    N = 20
    t_end = 200
    num_points = 2000
    phi_h = 0 (fixed)
...
Usage:
    python gamma_sweep.py --gamma 0.001 --output_dir results/
"""
...
def run_simulation(gamma_val, output_dir):
    """
    Run noise simulation for a single gamma value.
    """
    print("="*60)
    print("  GAMMA SWEEP")
    print("="*60)
    print(f"  gamma: {gamma_val:.6f} ({gamma_val:.6f} * 2pi)")
    ...
    gamma   = 2*np.pi*gamma_val  # Sweep parameter
    ...
    filename = f"gamma_sweep_gamma{gamma_val:.6f}.npz"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Gamma Sweep")
    parser.add_argument("--gamma", type=float, required=True, help="Gamma value (will be multiplied by 2pi)")
    ...
    run_simulation(args.gamma, args.output_dir)
