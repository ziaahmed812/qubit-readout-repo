#!/usr/bin/env python3
"""
Kappa=Epsilon Sweep - Set kappa=epsilon and sweep from 0.01 to 0.5 in steps of 0.05

Parameters:
    kappa   = epsilon = sweep parameter
    gamma   = 2*pi*0.001
    N = 20
    t_end = 200
    num_points = 2000
...
Usage:
    python kappa_epsilon_sweep.py --epsilon 0.05 --output_dir results/
"""
...
def run_simulation(epsilon_val, output_dir):
    """
    Run noise simulation for a single epsilon value (with kappa=epsilon).
    """
    print("="*60)
    print("  KAPPA=EPSILON SWEEP")
    print("="*60)
    print(f"  kappa = epsilon: {epsilon_val:.6f} * 2pi")
    ...
    # -- Parameters --
    kappa   = 2*np.pi*epsilon_val  # kappa = epsilon
    epsilon = 2*np.pi*epsilon_val  # Sweep parameter
    gamma   = 2*np.pi*0.001
    ...
    filename = f"kappa_epsilon_sweep_eps{epsilon_val:.6f}.npz"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Kappa=Epsilon Sweep")
    parser.add_argument("--epsilon", type=float, required=True, help="Epsilon value (kappa will equal this, multiplied by 2pi)")
    ...
    run_simulation(args.epsilon, args.output_dir)
